...
function normalizeStep(step) {
  ...
  return {
    ...
    conclusion: text(candidate.conclusion),
...
function qualification(status, observedJobs) {
  const observation = observedJobs === 0
    ? ' GitHub reported no workflow jobs for this completed run; no check execution is inferred.'
    : '';
  switch (status) {
    case 'passed': return `GitHub marked the workflow run successful. Individual job and step outcomes remain explicit, including skipped execution; no unexecuted check is inferred to have passed.${observation}`;
    case 'failed': return `GitHub marked the workflow run failed. Failed, skipped, cancelled, and incomplete job or step outcomes remain explicit.${observation}`;
    case 'cancelled': return `GitHub marked the workflow run cancelled. Work that did not complete is not represented as passed.${observation}`;
    case 'skipped': return `GitHub marked the workflow run skipped. No execution is inferred from the skipped run.${observation}`;
    default: return `The workflow run is incomplete or has a provider conclusion that does not establish successful execution. No incomplete work is represented as passed.${observation}`;
  }
}
...
if (!jobsPayload || !Array.isArray(jobsPayload.jobs) || !Number.isInteger(jobsPayload.total_count) || Number(jobsPayload.total_count) < 0) {
  throw new Error('GitHub returned an invalid workflow jobs response.');
}
const jobs = jobsPayload.jobs.map(normalizeJob);
...
const reportIdentity = { source: 'github.retained-reports', native: `${repository}|github.retained-reports|${id}` };
...
const report = {
  ...
  observedAt,
  status,
  qualification: qualification(status, jobs.length),
  checks: jobs,
  evidence: [
  ...
  relationships: [
    {
      relation: 'producedBy',
      from: reportIdentity,
      to: { source: 'github.workflow-runs', native: `${repository}|github.workflow-runs|${runId}`, revision: String(run.run_attempt) },
    },
    {
      relation: 'evaluatesRevision',
      from: reportIdentity,
      to: { source: 'github.commits', native: `${repository}|github.commits|${run.head_sha}`, revision: run.head_sha },
    },
  ],
};

const schemaPath = 'contracts/assurance/report.schema.json';
const schema = JSON.parse(fs.readFileSync(path.resolve(schemaPath), 'utf8'));
const errors = formatJsonSchemaErrors(
  outputPath,
  schemaPath,
  validateJsonSchema(report, schema, {
    schemaPath,
    loadSchema: createFileSchemaLoader(process.cwd()),
  }),
);
