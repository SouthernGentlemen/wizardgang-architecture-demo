...
describe('DEMO-277 site footer contract', () => {
  it('keeps Security reachable through the canonical registered route', () => {
    expect(pageSource).toContain("const securityRoute = localization.href(routeUrl('security.index'));\n");
    expect(pageSource).toContain('<span class="site-footer-links"><a href="${escapeHtml(securityRoute)}">');
  });

  it('renders version and commit proof through the shared versionProof projection', () => {
    expect(pageSource).toContain("import { versionProof } from './version-proof';");
    expect(homeSource).toContain("import { versionProof } from './version-proof';");
    expect(pageSource).toContain('const version = versionProof(env);');
    expect(homeSource).toContain('const version = versionProof(env);');
    expect(pageSource).not.toContain('DEPLOYED_VERSION');
    expect(pageSource).not.toContain('DEPLOYED_SHA');
  ...
  it('uses two intentional footer groups with meaningful space-between layout', () => {
    expect(pageSource).toContain('<span class="site-footer-links">');
    expect(pageSource).toContain('<span class="site-footer-build">');
