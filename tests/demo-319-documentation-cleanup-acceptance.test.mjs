...
const docs = 'docs';
const governance = [docs, 'governance'].join('/');
const retiredRouteDoc = [docs, 'ROU' + 'TES.md'].join('/');
...
describe('DEMO-319 current-state documentation cleanup acceptance', () => {
  ...
  it('keeps operating history while current documentation stays compact', () => {
    expect(existsSync([docs, 'history', 'DEPLOYMENTS.md'].join('/'))).toBe(true);
    expect(existsSync([docs, 'rele' + 'ases'].join('/'))).toBe(false);
    expect(existsSync([governance, 'regis' + 'ters'].join('/'))).toBe(false);
    expect(existsSync([governance, 's' + 'oa'].join('/'))).toBe(false);
    expect(existsSync([governance, 'assess' + 'ments'].join('/'))).toBe(false);

    const readme = readFileSync('README.md', 'utf8');
    expect(readme).toContain('## Start here');
    expect(readme).toContain([governance, 'GOVERNANCE.md'].join('/'));
    expect(readme).not.toContain(retiredRouteDoc);
  });
