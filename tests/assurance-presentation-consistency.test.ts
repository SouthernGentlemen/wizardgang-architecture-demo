...
describe('assurance presentation consistency', () => {
  it('keeps risk HTML and API selection aligned on canonical ids', async () => {
    ...
    const api = await (await assuranceRisksResponse(new Request(`https://demo.wizardgang.ai/api/reporting/risks?${query}`))).json() as { records: Array<{ id: string }>; derived: { count: number } };
  ...
  it('keeps compliance HTML and API selection aligned on canonical ids', async () => {
    ...
    const api = await (await assuranceComplianceResponse(new Request(`https://demo.wizardgang.ai/api/reporting/compliance?${query}`))).json() as { records: Array<{ id: string }>; derived: { count: number } };
  ...
  it('presents incidents and exercises from the same current records collection', async () => {
    ...
    const api = await (await assuranceIncidentsResponse(new Request('https://demo.wizardgang.ai/api/reporting/incidents'))).json() as { records: Array<{ id: string }>; derived: { count: number } };
