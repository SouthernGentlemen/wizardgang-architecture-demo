...
async function generateReport(run: Record<string, unknown>, jobs: Array<Record<string, unknown>> = [jobFixture()]) {
  ...
  try {
    const fixtures = {
      ...
      '/repos/SouthernGentlemen/wizardgang-architecture-demo/actions/runs/7001/attempts/2/jobs?per_page=100': { total_count: jobs.length, jobs },
    };
...
describe('canonical retained report production', () => {
  it('records a successful provider run without inventing a second status model', async () => {
    const result = await generateReport(
      runFixture({ conclusion: 'success' }),
      [jobFixture({
        conclusion: 'success',
        steps: [{ number: 1, name: 'validate', status: 'completed', conclusion: 'success', started_at: '2026-09-04T17:55:00Z', completed_at: '2026-09-04T18:00:00Z' }],
      })],
    );
    expect(result.status, result.stderr).toBe(0);
    expect(result.report).toMatchObject({ source: 'github.retained-reports', status: 'passed' });
    expect((result.report?.checks as Array<{ outcome: string }>)[0].outcome).toBe('passed');
  });
  ...
  it('preserves failed, skipped, and incomplete provider outcomes with run provenance', async () => {
    ...
    const report = result.report as {
      ...
      relationships: Array<{ relation: string; from: { native: string }; to: { native: string } }>;
    };
    ...
    expect(report.observedAt).toBe('2026-09-04T18:00:00Z');
    expect(report.evidence.map((entry) => entry.kind)).toEqual(['workflow-run', 'source-revision', 'job']);
    ...
    expect(report.relationships[0].from.native).toBe(`${repository}|github.retained-reports|RPT-CI-7001-A2`);
    expect(report.relationships[0].to.native).toBe(`${repository}|github.workflow-runs|7001`);
    expect(report.relationships[1].to.native).toBe(`${repository}|github.commits|1234567890abcdef1234567890abcdef12345678`);
  });

  it('preserves a skipped job and skipped step without inferring execution', async () => {
    const result = await generateReport(
      runFixture({ conclusion: 'success' }),
      [jobFixture({
        conclusion: 'skipped',
        started_at: null,
        completed_at: null,
        steps: [{ number: 1, name: 'conditional check', status: 'completed', conclusion: 'skipped', started_at: null, completed_at: null }],
      })],
    );
    expect(result.status, result.stderr).toBe(0);
    const checks = result.report?.checks as Array<{ outcome: string; steps: Array<{ outcome: string }> }>;
    expect(checks[0].outcome).toBe('skipped');
    expect(checks[0].steps[0].outcome).toBe('skipped');
  });
  ...
  it('represents a completed cancelled run with zero observed jobs', async () => {
    const result = await generateReport(runFixture({ conclusion: 'cancelled' }), []);
    expect(result.status, result.stderr).toBe(0);
    expect(result.report).toMatchObject({ status: 'cancelled', checks: [] });
    expect(result.report?.qualification).toContain('no workflow jobs');
  });

  it('maps an unknown completed provider conclusion to incomplete', async () => {
    const result = await generateReport(runFixture({ conclusion: null }));
    expect(result.status, result.stderr).toBe(0);
    expect(result.report?.status).toBe('incomplete');
  });

  it('accepts only the approved assurance-monitor event boundary', async () => {
    const accepted = await generateReport(runFixture({ name: 'Assurance Monitor', event: 'schedule' }));
    expect(accepted.status, accepted.stderr).toBe(0);
    expect(accepted.report?.reportType).toBe('assurance-monitor');
    const rejected = await generateReport(runFixture({ name: 'Assurance Monitor', event: 'push' }));
    expect(rejected.status).not.toBe(0);
    expect(rejected.stderr).toContain('scheduled or manually dispatched');
  });

  it('rejects a workflow from another repository and an unapproved producer', async () => {
    const wrongRepository = await generateReport(runFixture({ head_repository: { full_name: 'attacker/fork' } }));
    expect(wrongRepository.status).not.toBe(0);
    expect(wrongRepository.stderr).toContain('does not match the publishing repository');
    const unapproved = await generateReport(runFixture({ name: 'Deploy' }));
    expect(unapproved.status).not.toBe(0);
    expect(unapproved.stderr).toContain('not approved to produce retained reports');
  });
  ...
  it('rejects provider data that cannot satisfy the retained-report schema', async () => {
    const result = await generateReport(runFixture(), [jobFixture({
      steps: [{ number: 0, name: 'invalid', status: 'completed', conclusion: 'success' }],
    })]);
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain('Retained report failed schema validation');
  });
  ...
  it('stores only bounded AI evaluation metadata in D1 audit/log tables', () => {
    ...
    expect(source).toContain("const evaluationVersion = 'mcp-boundary-v1'");
