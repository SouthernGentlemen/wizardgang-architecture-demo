...
const governanceRoute = routeUrl('assurance.governance');

async function governancePage(path = governanceRoute) {
...
describe('governance page presentation', () => {
  ...
  it('filters to one canonical register and preserves the selection through pagination', async () => {
    const { response, html } = await governancePage(`${governanceRoute}?dataset=governance.records.access-classes&limit=1`);
    ...
    expect(html).toContain(`href="${governanceRoute}?dataset=governance.records.access-classes&amp;limit=1&amp;cursor=`);
  });

  it('ignores an unknown register instead of creating a second filtering contract', async () => {
    const { response, html } = await governancePage(`${governanceRoute}?dataset=unknown&limit=1`);
