...
REST writes accept either the managed operator bearer credential or a short-lived token derived from a validated identity session. Identity-derived tokens are limited to a server-derived visitor namespace. Public REST, GraphQL, and MCP reads share the explicit `demo:read` boundary, and authenticated GraphQL mutations cross the same normalized-principal policy. Webhook receivers verify signatures over the exact request body and reject replayed delivery IDs.
