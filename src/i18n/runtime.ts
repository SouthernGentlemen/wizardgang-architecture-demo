...
type PresentationCatalog = Readonly<Record<string, PresentationEntry>>;

function presentationResource(locale: ResourceLocale): Readonly<Record<string, string>> {
  return Object.freeze(Object.fromEntries(
    Object.entries(presentation as PresentationCatalog).map(([key, entry]) => [key, entry[locale]]),
  ));
}

export const localeResources = Object.freeze({
  en: Object.freeze({ ...enCore, ...presentationResource('en') }),
  es: Object.freeze({ ...esCore, ...presentationResource('es') }),
  fr: Object.freeze({ ...frCore, ...presentationResource('fr') }),
  de: Object.freeze({ ...deCore, ...presentationResource('de') }),
  ja: Object.freeze({ ...jaCore, ...presentationResource('ja') }),
  ar: Object.freeze({ ...arCore, ...presentationResource('ar') }),
});
...
export const localeNames: Readonly<Record<SupportedLocale, string>> = Object.freeze({
  en: 'English', es: 'Español', fr: 'Français', de: 'Deutsch', ja: '日本語', ar: 'العربية',
});
...
export const supportedLocales = Object.freeze(config.supportedLocales.filter(isSupportedLocale)) as readonly SupportedLocale[];
...
function createLocalization(locale: SupportedLocale, currentUrl: URL): LocalizationContext {
  ...
  const context: LocalizationContext = {
    ...
    currency: (value: number, currency: string = 'USD', options: Omit<Intl.NumberFormatOptions, 'style' | 'currency'> = {}) => new Intl.NumberFormat(locale, { ...options, style: 'currency', currency }).format(value),
...
export function resolveLocalization(request: Request): LocalizationContext {
  ...
  const locale = requested === null ? (cookieLocale(request) ?? defaultLocale) : (isSupportedLocale(requested) ? requested : defaultLocale);
...
export function bindLocalization(env: Env, localization: LocalizationContext): Env {
  ...
  Object.defineProperty(scoped, LOCALIZATION_CONTEXT, { configurable: false, enumerable: false, writable: false, value: localization });
  return scoped;
}
export function localizationForEnv(env: Env): LocalizationContext {
  return (env as LocalizedEnv)[LOCALIZATION_CONTEXT] ?? createLocalization(defaultLocale, new URL(`${DEFAULT_ORIGIN}/`));
}
export function localeQueryParameter(): string { return LOCALE_QUERY_PARAMETER; }
