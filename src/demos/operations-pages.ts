...
function operationalPage(env: Env, title: string, heading: string, content: string): Response {
  return shell(env, title, `${navigation()}<section><div class="eyebrow">Operations / public proof</div><h1>${escapeHtml(heading)}</h1></section>${content}`, { cacheControl: 'no-store' });
}
...
export async function renderDashboard(env: Env): Promise<Response> {
  ...
  return operationalPage(env, 'Operations Dashboard', 'Operations Dashboard', `
