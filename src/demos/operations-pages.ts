...
export async function billingContent(env: Env): Promise<PageContent> {
  ...
  return operationalPage(env, routeUrl('operations.usage'), 'Cloudflare Usage & Cost', 'Cloudflare Usage & Cost', 'Can the system respond safely to resource and cost pressure?', 'src/demos/operations-pages.ts', `
  <section class="operations-section" aria-labelledby="cost-degradation-heading">
    <p class="eyebrow">What this demonstrates</p><h2 id="cost-degradation-heading">Cost-aware degradation</h2>
    <p>Production systems need defined behavior as resource consumption approaches operational limits. This demonstration models progressive degradation while preserving access to operations.</p>
    <p>Normal and Warning allow optional Worker compute. Degraded pauses that workload; health checks and operational inspection remain accessible. Returning to Normal restores optional compute.</p>
  </section>
  <section class="operations-section guardrail-simulator"><div class="operations-section-heading"><div><p class="eyebrow">Controlled demonstration</p><h2>Cost guardrail simulator</h2></div><span class="${badgeClass(current.state)}">${escapeHtml(current.state)} · ${current.percent.toFixed(1)}%</span></div><p>Choose a scenario to change the demo’s synthetic budget state and observe its workload behavior. This does not change Cloudflare billing, thresholds, or account configuration.</p><p>The example scenarios use 40%, 75%, and 95% of a synthetic budget. Warning begins at 70%; degradation begins at 90%.</p><div class="guardrail-states"><div data-state="normal"><strong>Normal</strong><span>40% scenario</span><p>Optional workloads available.</p></div><div data-state="warning"><strong>Warning</strong><span>75% scenario</span><p>Warning state signals cost pressure. Optional compute remains available.</p></div><div data-state="degraded"><strong>Degraded</strong><span>95% scenario</span><p>Optional compute paused. Operations stay online.</p></div></div><div class="button-row" aria-label="Guardrail scenario"><button type="button" data-budget="normal" aria-pressed="${current.state === 'normal'}">Normal</button><button type="button" data-budget="warning" aria-pressed="${current.state === 'warning'}">Warning</button><button type="button" data-budget="degraded" aria-pressed="${current.state === 'degraded'}">Degraded</button></div><pre aria-live="polite" data-budget-output hidden></pre></section>
  <section class="operations-section" aria-labelledby="provider-telemetry-heading">
    <p class="eyebrow">Inspect the provider data</p><h2 id="provider-telemetry-heading">Live provider telemetry</h2>
    <p>These normalized Cloudflare observations report resource usage and billing availability separately from the synthetic simulator. Unavailable billing data means the telemetry source did not supply a usable cost value; it does not establish an application outage.</p>
    <p>Telemetry freshness and availability retain their reported states. Check <a href="${escapeHtml(routeUrl('operations.index'))}#health">service health</a> for application and dependency status.</p>
    ${liveState}
  </section>
