...
export async function renderDashboard(env: Env): Promise<Response> {
  ...
  <section class="grid" aria-label="Current operational state">
    ...
    <article class="card"><h2 class="eyebrow">Health</h2><p class="${statusClass(health.status)}">${escapeHtml(health.status)}</p><p>Worker: ${health.services.worker}; D1: ${health.services.d1}; R2: ${health.services.r2}; Durable Objects: ${health.services.durableObjects}.</p><a href="/dashboard/health">Inspect health</a></article>
