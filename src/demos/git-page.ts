...
const cards = [
  ...
  ['github.workflow-attempts', 'Workflow attempts', 'Native workflow attempts with provider run identity and outcome state.'],
...
export function gitContent(env: Env): PageContent {
  ...
  <section class="panel" id="source-of-truth"><div class="lab-heading"><div><p class="eyebrow">Shared reporting contract</p><h2>GitHub source of truth</h2></div><button type="button" data-evidence-refresh>Refresh evidence</button></div><p>Each row is a provider-native GitHub object normalized through the canonical reporting contract. This page intentionally requests a bounded sample; source completeness and provider availability are shown explicitly instead of presenting the sample as an export.</p><p data-evidence-meta aria-live="polite">Connecting to GitHub…</p></section>
  ...
  (()=>{
    ...
    const labels={
      ...
      'github.workflow-attempts':['name','status','conclusion','run_attempt'],
    ...
    const loadEvidence=async()=>{const meta=q('[data-evidence-meta]');meta.textContent='Loading bounded GitHub reporting pages…';try{const collections={...Object.fromEntries(Object.keys(labels).map((source)=>[source,source])),'github.workflow-attempts':'reports','github.branch-protection':'github.branch-protection'};const results=await Promise.all(Object.entries(collections).map(async([source,collection])=>{const response=await fetch('/api/reporting/'+encodeURIComponent(collection)+'?limit=5',{headers:{accept:'application/json'}});const payload=await response.json();if(!response.ok)throw new Error(payload.detail||payload.error||'unavailable');if(payload.contract!=='contracts/assurance/reporting.schema.json')throw new Error('unexpected reporting contract');return{source,payload}}));let partial=0;for(const{source,payload}of results){const availability=payload.availability?.[source]||payload.availability?.[payload.dataset]||'unavailable';const completeness=payload.query?.pagination?.completeness||'complete';if(completeness==='partial')partial+=1;if(source==='github.branch-protection')renderControls((payload.records||[])[0],availability);else renderSource(source,payload.records||[],availability,completeness)}meta.textContent='Canonical reporting contract · checked '+new Date().toLocaleString()+' · '+(partial?partial+' bounded/partial source(s)':'all requested sources complete')}catch(error){meta.textContent='Reporting unavailable. '+String(error);document.querySelectorAll('[data-evidence-status]').forEach((node)=>node.textContent='Source unavailable.');renderControls(null,'unavailable')}};
