...
export async function operationsAvailabilitySection(request: Request, env: Env): Promise<{ html: string; availabilityLabel: string; unexpected: number }> {
  ...
  return { availabilityLabel, unexpected: aggregate.unexpected, html: `<section class="operations-section" id="availability" aria-labelledby="availability-heading">
    <div class="operations-section-heading"><div><p class="eyebrow">Availability</p><h2 id="availability-heading">Measured service history</h2></div>${referenceDetails([{ label: 'Source', href: sourceUrl(env, 'src/api/operations.ts') }, { label: 'Docs', href: sourceUrl(env, 'docs/OPERATIONS.md') }], 'Provenance')}</div>
...
export async function operationsActivitySection(request: Request, env: Env, health: HealthSnapshot): Promise<{ html: string; recent: ApplicationLogRow[] }> {
  ...
  return { recent, html: `<section class="operations-section" id="activity" aria-labelledby="activity-heading">
    <div class="operations-section-heading"><div><p class="eyebrow">Activity</p><h2 id="activity-heading">Public-safe application activity</h2></div>${referenceDetails([{ label: 'Source', href: sourceUrl(env, 'src/lib/logs.ts') }, { label: 'Docs', href: sourceUrl(env, 'docs/OPERATIONS.md') }], 'Provenance')}</div>
...
async function usageSection(env: Env): Promise<string> {
  ...
  return `<section class="operations-section" id="usage" aria-labelledby="usage-heading">
    <div class="operations-section-heading"><div><p class="eyebrow">Usage &amp; cost</p><h2 id="usage-heading">Resource pressure and cost resilience</h2></div>${referenceDetails([{ label: 'Source', href: sourceUrl(env, 'src/lib/cloudflare-usage.ts') }, { label: 'Docs', href: sourceUrl(env, 'docs/OPERATIONS.md') }], 'Provenance')}</div>
...
function deploymentSection(env: Env): string {
  ...
  return `<section class="operations-section" id="deployment" aria-labelledby="deployment-heading"><div class="operations-section-heading"><div><p class="eyebrow">Deployment</p><h2 id="deployment-heading">Release evidence</h2></div>${referenceDetails([{ label: 'Workflow source', href: sourceUrl(env, '.github/workflows/deploy.yml') }, { label: 'Operations docs', href: sourceUrl(env, 'docs/OPERATIONS.md') }], 'Provenance')}</div><p>Verify the running release against version metadata, repository source, and CI history.</p><dl><dt>Version</dt><dd>${escapeHtml(version)}</dd><dt>Environment</dt><dd>${escapeHtml(env.DEPLOYMENT_ENVIRONMENT || 'local')}</dd><dt>Branch</dt><dd>${escapeHtml(branch)}</dd><dt>Commit</dt><dd><a href="${escapeHtml(commit)}">${escapeHtml(sha ? sha.slice(0, 7) : 'not supplied')}</a></dd></dl><div class="link-row"><a href="${escapeHtml(routeUrl('operations.version'))}">Version JSON</a><a href="${escapeHtml(commit)}">Commit ↗</a><a href="${escapeHtml(repoUrl(env))}/actions">Actions ↗</a><a href="${escapeHtml(repoUrl(env))}/releases">Releases ↗</a><a href="${escapeHtml(sourceUrl(env, 'docs/ROUTES.md'))}">Route docs ↗</a></div></section>`;
}
...
export async function operationsContent(request: Request, env: Env): Promise<PageContent> {
  ...
  const [activity, latestActivityRows, recentErrors] = await Promise.all([
    operationsActivitySection(request, env, health),
    recentApplicationLogs(env, { limit: 1 }),
    recentApplicationLogs(env, { limit: 200, level: 'error' }),
  ]);
  ...
  const lastDay = Date.now() - 86_400_000;
  const errorsLastDay = recentErrors.filter((row) => new Date(row.created_at).getTime() >= lastDay).length;
  ...
  <section class="operations-kpis" aria-label="Current operational state"><article><strong>${escapeHtml(overall)}</strong><span>${healthy} / ${services.length} dependencies healthy</span></article><article><strong>${availability.availabilityLabel}</strong><span>${availability.unexpected} unexpected intervals</span></article><article><strong>${errorsLastDay}</strong><span>errors / last 24h</span></article><article><strong>${escapeHtml(ageLabel(latestActivityRows[0]?.created_at ?? null))}</strong><span>latest public activity</span></article></section>
  <section class="operations-section" id="status" aria-labelledby="status-heading"><div class="operations-section-heading"><div><p class="eyebrow">Status</p><h2 id="status-heading">Current service posture</h2></div>${referenceDetails([{ label: 'Source', href: sourceUrl(env, 'src/api/operations.ts') }, { label: 'Docs', href: sourceUrl(env, 'docs/OPERATIONS.md') }], 'Provenance')}</div><p><strong>${escapeHtml(overall)}</strong> · observed ${escapeHtml(dateLabel(health.checkedAt))}</p><div class="usage-products">${Object.entries(health.services).map(([name, state]) => `<article class="usage-product"><div><strong>${escapeHtml(name)}</strong><span class="${badge(state)}">${escapeHtml(state)}</span></div></article>`).join('')}</div><div class="link-row"><a href="${routeUrl('operations.health')}">Health JSON</a><a href="${routeUrl('operations.version')}">Version JSON</a><a href="${routeUrl('operations.robots')}">robots.txt</a></div><details class="operations-inspection"><summary>Inspect operational policy</summary><p>Demo: <strong>${escapeHtml(control.state)}</strong> — ${escapeHtml(control.publicMessage)}</p><p>User-requested ChatGPT fetch: <strong>${escapeHtml(crawler.state)}</strong>. Model-training crawl remains blocked through robots policy.</p></details></section>
