...
function renderLocator(record: PresentedPublicEvidence): string {
...
function usedByLinks(record: PresentedPublicEvidence): string {
...
export function renderEvidenceDemo(request: Request, env: Env): Response {
  ...
  const records = presentedEvidenceRecords(env, origin);
  ...
  (() => {
    ...
    description: 'Searchable public assurance evidence with exact deployed-commit source resolution, reverse record usage, and explicit freshness semantics.',
  });
}
