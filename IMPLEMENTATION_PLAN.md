...
|---|---|---|---|---|
| **DEMO-254** | DOCS | `demo-254-mvp-demo-contract` | DEMO-253 | Establish this root source of truth, bind `AGENTS.md` to it, and align older frontend/planning docs with the MVP target. No runtime change. |
| **DEMO-255** | FEAT | `demo-255-minimal-public-shell` | DEMO-254 | Reduce primary navigation/homepage to Demos, Assurance, Source; demote Security to the support/footer boundary; add the compact operational proof strip without reproducing Operations. |
| **DEMO-256** | REFACTOR | `demo-256-curated-demos` | DEMO-255 | Reframe `/demos` around Data, APIs, Integrations, Identity, and MCP; move Edge/Workers/DO and Accessibility/i18n to secondary proof; make advanced detail opt-in while preserving stable fragments and machine contracts. |
| **DEMO-257** | REFACTOR | `demo-257-minimal-assurance` | DEMO-256 | Reduce `/assurance` to the four primary checks and focused evidence drill-down; remove risks/incidents/exercises/concerns/registry dumps and other management-system inventories from ordinary HTML without deleting canonical data/APIs. |
| **DEMO-258** | REFACTOR | `demo-258-retire-public-operations` | DEMO-257 | Retire the human `/operations` route as an ordinary 404, remove it from sitemap/discovery, and preserve operational APIs, collectors, retention, logs, protected controls, and the small proof already surfaced on home. |
| **DEMO-259** | TEST | `demo-259-mvp-acceptance` | DEMO-258 | Add regression/acceptance coverage that enforces the minimal public surface, absence of hidden management/operations dumps, working demo actions, source/security access, route retirement, accessibility, localization, and machine-contract preservation. |
| **DEMO-260** | BUILD | `release/v0.20.0` | DEMO-259 | Release the completed minimal-demo stack as v0.20.0 through the existing annotated-tag-only production workflow and verify the live public surface. |

The current open DEMO-248 through DEMO-253 stack is a prerequisite to this sequence. DEMO-254 branches from the current DEMO-253 head; subsequent MVP changes stack in the order above until the stack is landed.
...
- Release metadata records DEMO-254 through DEMO-259.
- Annotated `v0.20.0` tag reproduces and deploys the reviewed state.
