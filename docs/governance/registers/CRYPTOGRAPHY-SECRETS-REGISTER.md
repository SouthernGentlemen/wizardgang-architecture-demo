...
| CRY-013 | Identity-derived short-lived API access token | Applicable for authenticated visitor sandbox writes | Ten-minute encrypted bearer credential bound to the validated identity and server-derived sandbox namespace | Issued by the Worker from `IDENTITY_SESSION_SECRET`; held only by the requesting browser | Authenticated same-origin issuance, bounded claims, encryption, expiry, and runtime verification are enforced in `src/lib/identity-session.ts` | Met | identity flow change, token lifetime change, exposure, cryptographic algorithm change |
