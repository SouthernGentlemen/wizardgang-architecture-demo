...
<!-- GENERATED:governance-records:cryptography-secrets:start -->
...
| CRY-013 | Identity-derived short-lived API access token | Applicable for authenticated visitor sandbox writes | Ten-minute encrypted bearer credential bound to the validated identity and server-derived sandbox namespace | Token sealed from `IDENTITY_SESSION_SECRET`; namespace derived with `IDENTITY_AUDIT_HMAC_SECRET`; browser holds the short-lived token | Authenticated same-origin issuance, bounded claims, separate keyed namespace derivation, encryption, expiry, and runtime verification are enforced in source | Met | identity flow change, token lifetime change, exposure, cryptographic algorithm change |
...
| CRY-025 | SHA-256 non-account digests | Applicable | Payload fingerprints, opaque session/assertion identifiers, bounded comparison, and evidence | Application code / D1 evidence stores | Unkeyed SHA-256 remains limited to values that are not identity-provider account identifiers | Met | data/evidence model change, correlation/privacy review |
...
| CRY-029 | `IDENTITY_AUDIT_HMAC_SECRET` | Applicable when identity is enabled | HMAC-SHA256 derivation for identity audit correlation identifiers and visitor sandbox namespaces | Cloudflare Worker secret; local ignored `.dev.vars` | Application requires at least 32 UTF-8 bytes, applies separate HMAC domains, and keeps the value server-side | Met | identity audit/data model change, exposure, owner change, recovery, algorithm change |
| CRY-030 | `DEMO_API_TOKEN` (retired) | Retired / revoked 2026-09-18 | Historical operator API bearer credential; no current application capability reads or accepts it | Removed from the production Cloudflare Worker secret store | Names-only preflight found DEMO_API_TOKEN as the sole undeclared provisioned name with all 15 required names present; after approved deletion, all 15 required names remained and no undeclared names remained | Met | any proposal to reintroduce an operator API bearer credential or equivalent broad application write authority |
<!-- GENERATED:governance-records:cryptography-secrets:end -->
