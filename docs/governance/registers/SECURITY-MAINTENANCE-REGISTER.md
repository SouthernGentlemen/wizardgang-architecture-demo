...
| `VM-003` | Private vulnerability reporting | **Partial** | `SECURITY.md` instructs reporters not to expose sensitive vulnerabilities publicly and to use private vulnerability reporting when enabled. | Independently verify/configure the private reporting mechanism and retain a periodic availability check without publishing sensitive details. | Information Security Owner | Initial verification by 2026-12-02; annual/material change |
