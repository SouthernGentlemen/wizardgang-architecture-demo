...
|---|---|---|---|---|---|---|---|
| MON-001 | Release integrity | Production deployment traceability | Compare deployed version/SHA with controlled commit, tag/release, deployment record, and `/api/operations/version` evidence | Each production release; quarterly summary | Release owner | 100% traceable production deployments; aligns to SEC-OBJ-001 | Active |
...
The scheduled Assurance Monitor in `.github/workflows/assurance-monitor.yml` runs daily and on manual dispatch. It keeps the existing assurance-data, freshness, live security-policy, `security.txt`, reporting-link, and private-reporting checks. Requests made to live public endpoints identify the monitor with the browser-compatible `WizardGangAssuranceMonitor/1.0` user agent and the public repository URL. If Cloudflare marks the first identified request as a managed challenge, the monitor consumes that response and retries the same request once; normal edge protection remains enabled and no provider bypass rule is required. A failed run opens, reopens, or updates one `Assurance Monitor failure` tracking issue outside the Actions tab; the next successful run records recovery there and closes it. The Actions run remains the detailed execution evidence. After a monitor implementation change is merged, the first passing scheduled run is retained as the operating-evidence point rather than being backdated from branch validation.
