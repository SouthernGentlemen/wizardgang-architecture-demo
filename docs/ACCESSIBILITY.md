...
`/assurance?view=compliance&framework=wcag-2.2` is the human-readable projection of those criterion records. Its framework, status, and level filters use native labeled controls; each criterion row has a stable `WCAG-<criterionId>` anchor, links back to its canonical partition, and links each evidence ID to `/assurance?view=evidence`. The same normalized records are available from `GET /api/reporting/compliance?framework=wcag-2.2`, with exact lookup at `GET /api/reporting/compliance/WCAG-<criterionId>`. The projection derives counts at request time and does not add conformance meaning to the source status.
