...
`/assurance#accessibility-posture` provides a bounded human summary with a small sample of published WCAG evidence. The exhaustive normalized records are available from `GET /api/reporting/compliance?framework=wcag-2.2`, with exact lookup at `GET /api/reporting/compliance/WCAG-<criterionId>`. The public summary derives its posture at request time and does not add conformance meaning to the source status.
