...
Still outstanding before any conformance statement: browser/device verification of every applicable route state, keyboard-only traversal, screen-reader testing, 320 CSS-pixel and 400% zoom reflow, 200% text resize, text-spacing overrides, light/dark computed contrast, forced colors, reduced motion, Arabic RTL visual/focus order, touch target geometry, dynamic announcements, and any authenticated-demo state that requires a live environment. DEMO-238 is expected to formalize systematic release evidence and gates; DEMO-237 does not pre-claim those results.
