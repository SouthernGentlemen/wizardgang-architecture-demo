...
`/assurance/evidence` is the canonical searchable human evidence view. Stable record fragments remain the record IDs, so a record is addressed as `/assurance/evidence#<record-id>`. `GET /api/reporting` exposes the reporting collection index and `GET /api/reporting/evidence` exposes the evidence collection. Repository locators resolve only against the exact deployed commit identified by `DEPLOYED_SHA`; when that identity is unavailable, the projection reports `not-supplied` rather than falling back to `main`. Route evidence resolves against the current request origin.
...
`/assurance/compliance` is the assurance posture front door across those sources and links into the evidence view for record-level inspection. It states the WCAG 2.2, ISO/IEC 27001, and ISO/IEC 42001 posture as aligned or supported and uncertified, then links to the canonical working evidence at `/interfaces/accessibility`, `/interfaces/i18n`, `/assurance/governance`, `/interfaces/mcp`, `/assurance/delivery`, and `/operations`. It does not reproduce their explanations or turn alignment into a certification claim.

The public assurance browser uses `/assurance` as a real index with canonical children for delivery, governance, evidence, compliance, risks, incidents, and concerns. The former `/git`, `/governance`, `/evidence`, `/compliance`, `/concerns`, `/risks`, and `/incidents` HTML paths are intentionally unregistered and return the ordinary 404 without redirects. `/security` remains separate for private vulnerability reporting and published advisories.
