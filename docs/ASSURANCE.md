...
This separation does not authorize an API break. `docs/ASSURANCE-API.md` remains the released compatibility contract: stable routes, IDs, version-1 fields, deterministic collection ordering, filters, pagination behavior, and lifecycle presentation remain supported. Historical implementation details such as legacy-format lifecycle decoders are not part of that public contract and are removed after their evidence has been normalized.
...
`assurance/lifecycle/records.json` is the lifecycle control plane for every stable assurance record ID. The released `v0.14.0` registry at commit `c2359f00fc3bac80bfbc2e82369a86f20e522f74` remains the immutable historical source. Its verified membership is frozen in `src/assurance/generated/lifecycle-baseline-membership.json` at Git blob `e2f4aad3146649fdfe2063abc780dc73a99bdd64`. Commit `6f8383cd6a318e0fe03506bc96401f5161c6e222` is the pinned current-contract migration bridge: it was the last `main` state that successfully verified those historical identities through the legacy reader, and lifecycle validation now resolves the frozen IDs through that registry-driven current representation instead of decoding v0.14 formats again. Historical Git objects remain untouched.

Review facts are normalized into `reviewEvents` and referenced rather than copied. Lifecycle entries use `reviewRef` for the review event governing identity/state. `sourceApprovals` separately bind a review event to one registered resource and one exact Git blob revision. A stable record ID therefore does not grant publication permission to later content: changing any registered source file changes its blob revision, the generated runtime binding changes with it, and publication fails closed until that exact revision has an appropriate reviewed source approval. Where the source bytes are unchanged from the original approval event, the source approval reuses that original review reference.

Lifecycle values are `Draft`, `Approved`, `Published`, `Superseded`, and `Withdrawn`. Lifecycle state does not weaken the public disclosure boundary: because the canonical assurance package is public, every current or retained record must resolve to a reviewed lifecycle event before it can be committed here, and every current published projection must also resolve an approval for its exact source revision. Draft and approval state describe record governance, not permission to place sensitive material in the repository. Any stable ID outside the frozen historical membership requires explicit lifecycle metadata and receives no inherited approval.

Stable IDs are never recycled. Approved and published record identity anchors are immutable; if the represented record changes identity, create a new ID and use `supersedes` / `supersededBy`. A `Superseded` record requires at least one reciprocal `supersededBy` relationship. A `Withdrawn` record requires a meaningful `withdrawalRationale`. Removing a previously known record requires a retained `Superseded` or `Withdrawn` tombstone in `retiredRecords`; deleting both the record and its reservation is rejected by lifecycle validation. Once schema-version-2 lifecycle history exists, review events, source approvals, lifecycle reservations, and retired reservations are append-only across normal history.

The lifecycle validator compares current records with the frozen historical membership through the normalized migration bridge and with the previous current-contract repository snapshot. This prevents silent deletion, detects reuse of locked IDs for a different record identity, preserves retired ID reservations, requires explicit metadata for newly introduced IDs, verifies reciprocal supersession links, and rejects a current resource revision without a matching disclosure approval. Sensitive public fields such as credentials, private treatment detail, triage notes, exploit detail, reporter identity, or unreleased vulnerability detail are rejected independently of lifecycle state.
...
`npm run validate:assurance` includes lifecycle validation plus the cross-dataset integrity pass after the framework-specific validators. It fails closed on unreviewed public records, missing exact source-revision approval, unsupported lifecycle states, silent record deletion, immutable ID reuse, invalid supersession/withdrawal metadata, sensitive disclosure fields, globally duplicated public IDs, unsupported status vocabularies, missing ISO N/A rationales, unresolved evidence/risk/incident/advisory links, stale time-bound observation evidence, missing assurance routes, stored derived counts, public fields reserved for private or sensitive detail, stale generated SoA summaries, expired security-reporting metadata, and missing accountable owners.

`npm run test:assurance-integrity` runs mutation-based negative tests against isolated repository copies so each important failure mode is proven to return a non-zero validator result. Lifecycle tests explicitly prove the frozen membership artifact remains pinned, legacy decoders stay absent, unknown IDs receive no inherited lifecycle authority, changed source content requires a new exact-revision disclosure decision, retired IDs remain reserved, locked identities cannot be reused, and retirement/supersession rules remain strict. The targeted suite also verifies that HTML projections consume canonical relationship fields while representative version-1 responses retain their released compatibility aliases, record ordering, lifecycle metadata, and pagination behavior. The normal unit suite additionally proves generated-summary drift rejection, owner/expiry gates, deterministic release snapshot metadata/digests, registry/schema-driven filters, resolver-backed links, and reverse evidence completeness.

Run `npm run validate:assurance` to enforce paths, identifiers, visibility, schemas, lifecycle, disclosure review, exact source-revision approval, immutable identity, retention, duplicates, referential integrity, freshness, public disclosure boundaries, route coverage, generated SoA consistency, operational ownership, reporting metadata, and the separation between canonical assurance data and derived presentation fields.
